...
import { useCompositeTuner, type CompositeResult } from "./useCompositeTuner";
import { useAutoCompositeTuner, type AutoCompositeResult } from "./useAutoCompositeTuner";
...
import Composite3Console from "./Composite3Console";
import { exportToPdf, exportToImage } from "@/lib/tuner/exportPdf";
import { useAuth } from "@/hooks/useAuth";
import { useUserRole } from "@/hooks/useUserRole";
import SectionTabs from "./SectionTabs";
import TargetNoteBar from "./TargetNoteBar";
import { COMPOSITE2_SECTION_ORDERS, useManualSequence } from "./useManualSequence";
import {
  appendHighRepeatSample,
  resolveHighRepeatConsensus,
  type HighRepeatConsensus,
  type HighRepeatSample,
} from "./highRepeatConsensus";
...
export default function NewTabPage() {
  ...
  // 신규탭 = run-tune 레포의 "복합2" 탭을 그대로 가져온 것.
  // 원본은 경로(/composite2)로 모드를 판별했지만, 이 앱에서는 항상 복합2 모드로 고정한다.
  const isComposite3 = false;
  const isComposite2 = true;
  const { user } = useAuth();
