// Microphone capture. Web preview uses the Web Audio API (getUserMedia + AnalyserNode)
// which runs inside the Expo web preview. Native mic streaming is added later; on
// native `start()` reports an informative error so the UI can show a notice.
...
export function useAudioAnalyzer(onFrame: FrameCallback) {
  ...
  const start = useCallback(async () => {
    ...
    if (!isMicSupported()) {
      setError(
        Platform.OS === "web"
          ? "이 브라우저는 마이크 접근을 지원하지 않습니다."
          : "네이티브 마이크 캡처는 준비 중입니다. 웹 프리뷰에서 사용해 주세요.",
      );
      return;
    }
    try {
